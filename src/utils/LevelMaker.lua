...
function LevelMaker.createMap(level)
    ...
    if level > 1 then
        -- set slight chance of having locked brick
        hasLockedBrick = math.random(0, 5 * level) < 5 * level * LOCKED_BRICK_SPAWN_RATE
    end
    ...
    if #bricks == 0 then
        local newLevel = self.createMap(level)
    else
        local powerups = {}
        if hasLockedBrick then
            local lockedBrick = {}
            local lockedBrickIdx = math.random(1, #bricks)
            local keyBrickIDx = math.random(1, #bricks)
            while lockedBrickIdx == keyBrickIDx do
                keyBrick = math.random(1, #bricks)
            end
            for i, brick in pairs(bricks) do
                if i == lockedBrickIdx then
                    brick.color = 6
                    brick.tier = 1
                    lockedBrick = brick
                elseif i == keyBrickIDx then
                    local p = Powerup({
                        x = brick.x + 8,
                        y = brick.y,
                        brick = brick,
