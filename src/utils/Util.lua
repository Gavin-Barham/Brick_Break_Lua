...
function GenerateQuadsBricks(atlas)
    ...
    local tableSlice = table.slice(quads, 1, 21)
    local lastBrick = table.slice(quads, 24, 25)
    table.insert(tableSlice, lastBrick[1])

    return tableSlice
end

function GenerateQuadsPowerUps(atlas)
    local quads = GenerateQuads(atlas, 16, 16)

    local tableSlice = table.slice(quads, 144, 154)

    return {tableSlice[10], tableSlice[11]}
end
--[[
